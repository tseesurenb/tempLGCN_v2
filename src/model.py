...
class tempLGCN_attn(MessagePassing):    
    def __init__(self, 
        ...
        if self.u_abs_drift:
            ...
            self._u_abs_beta_emb = nn.Embedding(num_embeddings=num_users, embedding_dim=1).to(self.device)  
        ...
        if self.u_rel_drift:
            ...
            self._u_rel_beta_emb = nn.Embedding(num_embeddings=num_users, embedding_dim=1).to(self.device)  
    ...
    def forward(self, edge_index: Tensor, src: Tensor, dest: Tensor, u_abs_decay: Tensor, u_rel_decay: Tensor, i_rel_decay: Tensor):
        ...
        if self.u_abs_drift:
            ...
            abs_decay = torch.sigmoid(u_abs_decay.unsqueeze(1) * self._u_abs_beta_emb.weight[src])
            #abs_decay = torch.log1p(u_abs_decay.unsqueeze(1) * self._u_abs_beta_emb.weight[src])
        ...
        if self.u_rel_drift:
            ...
            rel_decay = torch.sigmoid(u_rel_decay.unsqueeze(1) * self._u_rel_beta_emb.weight[src])
            #rel_decay = torch.log1p(u_rel_decay.unsqueeze(1) * self._u_rel_beta_emb.weight[src])  # Apply learned beta using log1p
            #rel_decay = 1/torch.pow(u_rel_decay.unsqueeze(1), self._u_rel_beta_emb.weight[src])
