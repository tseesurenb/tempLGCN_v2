...
class tempLGCN_attn(MessagePassing):    
    ...
    def forward(self, edge_index: Tensor, src: Tensor, dest: Tensor, u_abs_decay: Tensor, u_rel_decay: Tensor, i_rel_decay: Tensor):
        ...
        if self.u_abs_drift:
            ...
            abs_decay = torch.tanh_(u_abs_decay.unsqueeze(1) * self._u_abs_beta_emb.weight[src])
        ...
        if self.u_rel_drift:
            ...
            rel_decay = torch.tanh_(u_rel_decay.unsqueeze(1) * self._u_rel_beta_emb.weight[src])
